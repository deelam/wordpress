...
to set credentials like so: ./alegion_sdk.py cred MY_REALM 'MY_USERNAME' 'MY_PASSWORD'
...
For debugging purposes, the alegion-cred.json file is not encrypted so the password is revealed.
Delete the alegion-cred.json file at the end of your session if desired.

Authentication will be automatically done for each action if the access_token is expired. 
The alegion-auth.json file will be updated with the authentication response from the server
and will be used for all actions.
